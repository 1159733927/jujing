...
describe('Harness report prompt', () => {
  it('passes complete birth facts to the report Agent so it cannot claim the birth hour is missing', () => {
    ...
    expect(prompt).toContain('五行分布参考（按显性天干和地支本气归类）')
    expect(prompt).toContain('可用于解释住宅方位与个人气质的匹配倾向')
    expect(prompt).toContain('月令参考：申月；主气庚（金）；对日主十神为偏财')
    expect(prompt).toContain('这是命盘事实，可用于解释五行气势，但不要写成绝对吉凶')
    expect(prompt).toContain('得令、得地、得助线索：月令主气不扶助日主')
    expect(prompt).toContain('同类根位于日支')
    expect(prompt).toContain('印星透干位置：时干')
    expect(prompt).not.toContain('不等同于完整旺衰、格局或喜忌结论')
    expect(prompt).not.toContain('不得自动升级为身强、身弱、从格、具体格局、喜神、忌神或用神结论')
  ...
  it('includes user-facing citation labels and the complete evaluated rule result', () => {
    ...
    expect(prompt).toContain('格局：本次没有可直接写给用户的结论；报告不用单独提这一项。')
    expect(prompt).toContain('神煞：本次没有可直接写给用户的结论；报告不用单独提这一项。')
    expect(prompt).not.toContain('程序层本次暂无确定结论')
    expect(prompt).not.toContain('不得自行断定身强、身弱、从格、具体格局、喜神、忌神或用神')
