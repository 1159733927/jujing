...
${CULTURAL_USE_NOTICE}`
  ...
  it('keeps the low-risk action notice out of the lead paragraph and deduplicates it', async () => {
    vi.stubEnv('DEEPSEEK_API_KEY', 'test-deepseek-key')
    const record: ReportRecord = {
      ...baseRecord,
      compatibility: {
    ...
    const modelReport = `结论先说：这套房和你的命盘局部合拍，南侧厨房是主要加分项，中宫附近卫生间是主要扣分项，先处理卫生间湿气。以上建议都只涉及低成本、可撤销的日常布置调整。

南侧厨房与丙火日主存在火性呼应。命盘依据：日主为丙火，四柱完整。住宅依据：住宅整体朝南，厨房在南侧。这是这套房比较值得保留的地方。

中宫附近卫生间会削弱住宅中心稳定感。命盘这一侧更需要稳定感；卫生间靠近户型中心区域，所以这是最明显的冲突点。
...
${CULTURAL_USE_NOTICE}`
    ...
    const result = await generateReportWithRunner(record, runner)
    const notice = '以上建议都只涉及低成本、可撤销的日常布置调整。'
    const lead = result.report.split(/\n\s*\n/u)[0]

    expect(result.generationProvenance).toMatchObject({ validatorResult: 'pass' })
    expect(lead).not.toContain(notice)
    expect((result.report.match(new RegExp(notice, 'gu')) ?? []).length).toBe(1)
