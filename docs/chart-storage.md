...
- `GET /v1/shared-reports/:id/chart-pdf`：同样只接受 `x-report-share-token`，用于只读分享页导出该报告绑定的命盘 PDF。接口复用报告记录里已固化的出生输入、命盘计算结果和生成时间，不重新排盘；鉴权、过期、归档、质检状态与分享正文读取一致，手工四柱报告暂不支持命盘 PDF 导出。
