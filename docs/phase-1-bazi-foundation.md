...
| Geography data | The national administrative tree is present through `province-city-china@8.5.8`. A dated GeoNames China snapshot is bundled after conservative crosswalk review: 2612/3311 districts have reviewed GeoNames coordinates, plus 2 explicit `manual-demo` fallbacks. For investor-demo usability, 667 districts without exact records now use explicitly marked `city-derived` coordinates from same-city usable records, for 3281 selectable districts and 30 unavailable districts. Persisted chart/report writes and the expert WenZhen capture form require `placeCode`; the server re-derives coordinates, timezone and dataset version. | Expand exact district coverage only through reviewed, licensed records; expose attribution, hashes, city-derived counts and unavailable counts; never imply nationwide complete district-level coordinates. |
...
The current product package is labeled `licensed-partial`: it is suitable for an investor demo and reproducible testing, but not for claiming complete district-level China coordinate coverage. The old hand-curated demo subset is retained only as explicitly marked `manual-demo` fallback records when no reviewed GeoNames record exists. Districts without exact records may be selectable only when they carry explicit `city-derived` evidence from same-city usable coordinates; the UI must present that as city-level estimation rather than reviewed district precision.
