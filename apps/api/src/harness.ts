...
function buildOutputContract(): string {
  return [
    '第一段必须像真人顾问一样直接下判断，第一句话必须以“结论先说：”开头，直接回答这个人与这套住宅是否合拍：这套住宅与这个命盘整体是合拍、局部合拍、有合有冲、冲突偏多，还是暂不能判断；同一段必须点出最大的加分项、最大的短板或冲突，以及最该先处理的一件事。',
    '首段要让用户马上有抓手：可以自然写出“适合度约 7/10”“中上”“局部合拍但有一处短板”这类判断，但不要伪装成科学评分；评分或等级必须服务于判断，不要单独做表格。',
...
function buildQualityReviewPrompt(record: ReportRecord, draft: ReportDraft, attempt: number): string {
  return [
    ...
    '8. 普通用户应能很快看懂整体是否合拍和关键原因。若首段没有实际加分、短板、冲突或最该先做的动作，或者堆叠术语、反复同一意思，或出现 AI、审计、管道、结构化数据、测试档案、测试数据、QA、生成过程等口吻，应标 high。不得因没有固定标题、固定篇幅或固定条数而扣分。',
    ...
    '10. 若证据只支持厨房在南侧，却把阳台、门窗、采光面也写成南侧，或把不同住宅、不同照片的空间事实合并到同一套房，应标 high。',
    '11. 建议拆墙、改承重结构、封门窗、改燃气水电、搬家，或宣称建议必然转运、发财、治病、改变婚育，应标 high。',
    '12. 若已经能够形成整体人宅判断，却把正文重点写成“待确认信息”“证据不足”“判断前提与可信度”“命盘需要”“住宅属性”“资料清单”“依据与版本”“引用依据”“资料来源”等用户无感内容，应标 high；若反复用“待确认、信息不足、后续再看”稀释结论，也应标 high。',
    '13. 如果没有一段像“可以先这样做”的用户行动建议，或行动建议少于 2 个具体动作，应标 high。',
...
function buildReportRevisionPrompt(record: ReportRecord, draft: ReportDraft, review: ReportQualityReview, nextAttempt: number): string {
  return [
    ...
    '修订后第一句话必须以“结论先说：”开头，首段给出整体合拍判断、最大加分项、最大扣分项或冲突，以及最该先处理的一件事，让普通用户很快看懂关键原因，并获得针对该住宅的低风险可撤销建议。必须有一段“## 可以先这样做”，写 2 到 4 条具体动作，且不要出现“基线、候选方向”等内部词。结构和篇幅由你决定；修订后仍会经过服务端安全与证据校验。',
    `本次修订轮次：${nextAttempt}`,
    '质量审核结果：',
